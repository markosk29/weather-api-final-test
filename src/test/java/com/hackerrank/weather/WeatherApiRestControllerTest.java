...
public class WeatherApiRestControllerTest {
    ...
    public void testWeatherEndpointWithPOST() throws Exception {
        WeatherJSON expectedRecord = getTestData().get("chicago");
    ...
    public void testWeatherEndpointWithGETList() throws Exception {
        Map<String, WeatherJSON> data = getTestData();
        data.remove("moscow2");
        List<WeatherInput> expectedRecords = new ArrayList<>();

        for (Map.Entry<String, WeatherJSON> kv : data.entrySet()) {
    ...
    public void testWeatherEndpointWithGETListAndCityFilter() throws Exception {
        ...
        for (Map.Entry<String, WeatherJSON> kv : getTestData().entrySet()) {
        ...
        List<WeatherInput> actualRecords = om.readValue(mockMvc.perform(get("/weather?city=moscow"))
                .andDo(print())
                .andExpect(jsonPath("$.*", isA(ArrayList.class)))
                .andExpect(jsonPath("$.*", hasSize(expectedRecords.size())))
                .andExpect(status().isOk()).andReturn().getResponse().getContentAsString(), new TypeReference<List<WeatherInput>>() {
    ...
    public void testWeatherEndpointWithGETListAndDateOrder() throws Exception {
        ...
        List<WeatherInput> actualRecords = om.readValue(mockMvc.perform(get("/weather?sort=date"))
                .andDo(print())
                .andExpect(jsonPath("$", isA(ArrayList.class)))
                .andExpect(jsonPath("$", hasSize(expectedRecords.size())))
                .andExpect(status().isOk()).andReturn().getResponse().getContentAsString(), new TypeReference<List<WeatherInput>>() {
    ...
    public void testWeatherEndpointWithGETById() throws Exception {
        ...
        WeatherInput actualRecord = om.readValue(mockMvc.perform(get("/weather/" + expectedRecord.getId())
                .contentType("application/json"))
                .andDo(print())
                .andExpect(status().isOk()).andReturn().getResponse().getContentAsString(), WeatherInput.class);
    ...
    private Map<String, WeatherJSON> getTestData()  {
        Map<String, WeatherJSON> data = new LinkedHashMap<>();

        WeatherJSON chicago = new WeatherJSON(
                "2019-06-11",
                41.8818f,
                -87.6231f,
                "Chicago",
        ...
        WeatherJSON oakland = new WeatherJSON(
                "2019-06-12",
                37.8043f,
                -122.2711f,
                "Oakland",
        ...
        WeatherJSON london = new WeatherJSON(
                "2019-03-12",
                51.5098f,
                -0.1180f,
                "London",
        ...
        WeatherJSON moscow1 = new WeatherJSON(
                "2019-03-12",
                55.7512f,
                37.6184f,
                "Moscow",
        ...
        WeatherJSON moscow2 = new WeatherJSON(
                "2019-03-12",
                55.7512f,
                37.6184f,
                "Moscow",
